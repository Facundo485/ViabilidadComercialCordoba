...
def construir() -> dict:
    ...
    rasgos = []
    for fila in ancho.iter_rows(named=True):
        anillo = geo.get(fila["manzana"])
        ...
        rasgos.append(
            {
                ...
                "properties": {
                    "m": fila["manzana"],
                    "b": fila["barrio"],
                    "n": int(fila["hab_total"] or 0),
                    # Los scores van como array indexado por rubro, no como
                    # claves con nombre: repetir "bar_restaurante" 6.892 veces
                    # pesaba un megabyte entero. Y como enteros por mil, que es
                    # más precisión de la que el modelo tiene para dar.
                    "s": [round((fila[r] or 0) * 1000) for r in rubros],
                },
            }
        )
...
def ejecutar() -> dict:
    ...
    print(f"\n{'=' * 72}\n  Datos del mapa\n{'=' * 72}")
    print(f"\n{len(datos['manzanas']['features']):,} manzanas | {len(datos['rubros'])} rubros")
